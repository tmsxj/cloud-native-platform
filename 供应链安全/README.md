...
> 与已有的 Trivy 扫描、Kyverno 准入、Falco 运行时检测拼成完整 DevSecOps 四层。

## 做了什么
- **镜像签名**：`cosign` 用本地私钥对镜像签名（[`cosign-sbom/sign-and-sbom.sh`](./cosign-sbom/sign-and-sbom.sh)）
- **SBOM 生成**：`syft` 生成软件物料清单，随镜像留存
- **准入验签**：`Kyverno` `verifyImages` 策略在 Pod 准入时校验签名，**未签名镜像一律拒绝**
