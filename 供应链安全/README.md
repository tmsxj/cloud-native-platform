...
## DevSecOps 四层安全闭环（本项目已拼齐）

| 层 | 工具 | 职责 | 本项目状态 |
|----|------|------|-----------|
| 镜像扫描 | Trivy | 构建/入库前扫 CVE（常见漏洞与暴露） | 已具备扫描能力 |
| 准入控制 | Kyverno（策略即代码引擎） | 集群入口拦截违规/未签名镜像 | 已部署，`require-signed-images` 生效 |
| 镜像签名 | cosign（镜像签名工具） + SBOM（软件物料清单） | 给可信镜像签名、生成物料清单 | 本文件落地 |
| 运行时检测 | Falco | 运行时内核层异常行为告警（如读 `/etc/shadow`） | 已部署，告警闭环 |

> 四层覆盖「构建 → 准入 → 签名 → 运行」全链路：签名验签确保**只准跑可信镜像**，运行时检测兜底**即使漏网也能发现异常**。
