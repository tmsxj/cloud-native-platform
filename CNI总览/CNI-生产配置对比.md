# CNI 生产配置对比（Cilium vs Calico）

> 创建日期：2026-07-09
> 目的：面对不同生产环境时，快速选型 CNI 并定位所需配置调整
> 配套：`../eBPF-可观测性/Cilium-生产配置指南.md`、`../eBPF-可观测性/故障排查手册.md`、`../Calico-配置指南/`
...
- 全家桶（ES/Prometheus/Tomcat 等）不依赖特定 CNI，切换无影响；回退依据见 `../eBPF-可观测性/backup/`
...
3. 删除 Calico：`kubectl delete -f calico.yaml`（或对应 DaemonSet / Deployment / CRD）
