# 服务网格对比演示：Linkerd vs Istio（第 23 项）

> 目的：在同一套 kubeadm 集群（v1.28.15，聚焦模式，master 内存红线）上**先后落地两套服务网格**，
> 用**完全相同的 demo 应用**（nginx-backend + client）对比两者在 mTLS、注入、L7 观测、流量治理上的差异。
...
| 镜像策略 | 一律 `外网资源同步/sync_from_us.ps1` → Harbor `192.168.1.61`；YAML 内 image 写 `192.168.1.61/<项目>/<镜像>:<tag>` |
| 控制面落点 | Linkerd / Istio 控制面 Deployment 无 master toleration，天然只落 worker |
| 模式 | 聚焦模式：全家桶 scale 0，仅控制面 + Cilium + MinIO + 网格在跑 |
...
| 数据面 | Rust **proxy**（~10-20MB/sidecar） | C++ **Envoy**（~50-100MB/sidecar） |
...
|------|-------------------|-----------------|
| Sidecar 注入 | Pod 2/2（app + linkerd-proxy） | Pod 2/2（app + istio-proxy） |
...
| 观测入口 | linkerd-viz web + tap（已装） | 默认无 UI（需 Kiali/Grafana，本部署未装 addon；原始计数器已可采） |
